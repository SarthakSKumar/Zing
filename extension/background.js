...
// ── History scrub ─────────────────────────────────────────────────────────────
const FALLBACK_WORDS = ["dangerous"];

async function scrubHistory() {
  const s = await fetchSettings();
  if (s?.historyScrub?.enabled === false) return; // scrub disabled
  const words = s?.historyScrub?.words?.length
    ? s.historyScrub.words
    : FALLBACK_WORDS;
  try {
    ...
    console.warn("[Zing] Scrub failed:", e);
  }
}
  ...
  if (changeInfo.status === "complete") {
    ...
    try {
      ...
      if (d[`darkReader_${tabId}`]) {
        chrome.scripting
          .insertCSS({ target: { tabId }, css: DARK_READER_CSS })
          .catch(() => {});
      }
    } catch {}
  }
...
async function saveTabs(name) {
  ...
  try {
    ...
    const r = await (
      await fetch(`${API}/api/collections/${c.id}/urls`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "GET_DARK_READER_STATE") {
    ...
    chrome.storage.local
      .get(`darkReader_${tabId}`)
      .then((d) => sendResponse({ enabled: !!d[`darkReader_${tabId}`] }));
    return true;
  }
  if (msg.type === "DARK_READER_ESC") {
    ...
    chrome.storage.local.get(`darkReader_${tabId}`).then((d) => {
      if (d[`darkReader_${tabId}`]) {
        ...
        chrome.scripting
          .removeCSS({ target: { tabId }, css: DARK_READER_CSS })
          .catch(() => {});
      }
    });
  ...
  if (msg.type === "CLEAR_PESTICIDE_STATE") {
    ...
    if (tabId) {
      ...
      chrome.scripting
        .removeCSS({ target: { tabId }, css: PESTICIDE_CSS })
        .catch(() => {});
    }
    sendResponse({ ok: true });
    return false;
  ...
  if (msg.type === "CLEAR_COLOR_PICKER_STATE") {
    const tabId = sender.tab?.id;
    if (tabId)
      chrome.storage.session.remove(`colorPicker_${tabId}`).catch(() => {});
    sendResponse({ ok: true });
    return false;
  }
